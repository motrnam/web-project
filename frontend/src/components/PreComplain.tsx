...
const PreComplaintView: React.FC<PreComplaintViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'complainants' | 'reviews' | 'timeline' | 'documents'>('details');
  ...
  const getCrimeTypeColor = (type: CrimeType): { bg: string; text: string; label: string } => {
    switch (type) {
      case CrimeType.TYPE_3:
        return { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-800 dark:text-green-300', label: 'سطح ۳ - جرائم خرد' };
      case CrimeType.TYPE_2:
        return { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-800 dark:text-yellow-300', label: 'سطح ۲ - جرائم متوسط' };
      case CrimeType.TYPE_1:
        return { bg: 'bg-orange-100 dark:bg-orange-900/30', text: 'text-orange-800 dark:text-orange-300', label: 'سطح ۱ - جرائم سنگین' };
      case CrimeType.CRITICAL:
        return { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-800 dark:text-red-300', label: 'بحرانی' };
      default:
        return { bg: 'bg-gray-100 dark:bg-gray-700', text: 'text-gray-800 dark:text-gray-300', label: type };
    }
  };
      ...
      case 'cadet':
        return (
          <div className="flex flex-wrap gap-2">
            ...
            {complaint.status !== ComplainStatus.REJECTED && complaint.status !== ComplainStatus.APPROVED && (
              <button
      ...
      case 'supervisor':
        return (
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={() => onReview ? onReview({}) : console.warn("onReview function not provided")}
              className="
                flex items-center gap-2
                px-4 py-2
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden" dir="rtl">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getStatusBadgeColor(complaint.status)}`}>
                {complaint.status}
              </span>
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${crimeTypeStyle.bg} ${crimeTypeStyle.text}`}>
              ...
              <span className="inline-flex items-center gap-1 text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>{userRole === 'complainant' ? 'شاکی' :
                  userRole === 'cadet' ? 'کارآموز' :
                    userRole === 'officer' ? 'افسر' :
                      userRole === 'supervisor' ? 'ناظر' : 'مدیر'}</span>
              </span>
            </div>
          </div>
      ...
      <div className="px-6 border-b border-gray-200 dark:border-gray-700">
        <nav className="flex gap-6">
          {(['details', 'complainants', 'reviews', 'timeline', 'documents'] as const).map((tab) => (
            <button
              key={tab}
